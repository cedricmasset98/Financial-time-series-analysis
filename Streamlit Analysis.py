...
def apply_plotly_layout(fig, title=None, xaxis_title=None, yaxis_title=None, height=None, showlegend=True):
    ...
    if title:
        updates['title'] = dict(text=title, font=dict(size=18))
    if xaxis_title:
...
with tab_cours:
    fig_price = make_subplots(
        ...
        subplot_titles=[f'Stock Price {actif} ({freq})', 'Drawdown']
    )
...
with tab_logprice:
    ...
    subtitles = [f'Log-Price {actif} with linear regression ({freq})']
    if show_ma:
        subtitles.append('Deviation (Log-Price - MA)')
    ...
    st.plotly_chart(fig_lp, key="chart_logprice")
...
with tab_return:
    fig_ret = make_subplots(
        ...
        subplot_titles=[f'Returns {actif} ({freq})', 'Returns histogram (%)']
    )
    ...
    st.plotly_chart(fig_ret, key="chart_returns")
...
with tab_scatter:
    ...
    apply_plotly_layout(fig_sc,
                        title=f'Scatter plot: Return t vs t-1 ({freq})',
                        xaxis_title='Return t-1',
                        yaxis_title='Return t',
                        height=650)
    st.plotly_chart(fig_sc, key="chart_scatter")
...
with tab_moments:
    ...
    if st.button("Compute rolling moments", key="btn_moments"):
        ...
        fig_mom = make_subplots(
            rows=2, cols=2,
            subplot_titles=['Time-varying mean', 'Time-varying volatility',
                            'Time-varying skewness', 'Time-varying kurtosis'],
        ...
        st.plotly_chart(fig_mom, key="chart_moments")
...
with tab_qq:
    ...
    fig_qq = make_subplots(
        ...
        subplot_titles=['QQ-Plot of returns', 'ACF of returns', 'PACF of returns'],
        vertical_spacing=0.12,
    ...
    st.plotly_chart(fig_qq, key="chart_qq")
...
with tab_cumret:
    ...
    if selected_assets:
        ...
        apply_plotly_layout(fig_cr,
                            title='Comparative Cumulative Return',
                            xaxis_title='Date',
                            yaxis_title='Cumulative Return',
                            height=550)
        st.plotly_chart(fig_cr, key="chart_cumret")
    else:
    ...
    if st.button("Compute rolling return", key="btn_rolling"):
        ...
        if df_roll is not None:
            ...
            fig_roll = make_subplots(
                ...
                row_heights=[0.65, 0.35],
                vertical_spacing=0.1,
                subplot_titles=[f'Rolling Return {actif} ({window_label})', 'Rolling returns distribution']
            )
            ...
            st.plotly_chart(fig_roll, key="chart_rolling")