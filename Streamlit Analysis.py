...
with tab_cours:
    ...
    apply_plotly_layout(fig_p, title=f'Stock Price {actif} ({freq})', yaxis_title='Close', height=450)
...
with tab_logprice:
    ...
    # 2. Graphique de déviation (si MA activée)
    if show_ma:
...
with tab_return:
    ...
    st.markdown("<h5 style='text-align: center;'>📐 Student-t Fit Parameters</h5>", unsafe_allow_html=True)
    m_col1, m_col2, m_col3 = st.columns(3)
    m_col1.metric("Degrees of freedom (df)", f"{df_fit:.2f}")
    m_col2.metric("Mean (μ)", f"{loc:.4f}")
    m_col3.metric("Scale (σ)", f"{scale:.4f}")
